...
errstr = """\
`common_files.treat.materials` has been deprecated.
Please use `materials` instead."""
raise DeprecationWarning(errstr)

#try:
#  from corebuilder.primitives import Material as XMLMaterial
#except ImportError:
#  import sys
#  import inspect
#  file_ = os.path.realpath(inspect.getfile(inspect.currentframe()))
#  sys.path.append(os.path.join(os.path.dirname(file_),'..'))
#  from corebuilder.primitives import Material as XMLMaterial
...
#parser = OptionParser()
#parser.add_option('--latex', dest='latex', action="store_true", default=False)
#parser.add_option('-d', dest='dir', default=None)
...
wBpFuels = [0.0000059,0.0000076] # 5.9 ppm boron by weight is the recommended boron
                       # impurity in the fuel in BATMAN, but 7.6 ppm is also cited in the original report
wBpGraphite = [0.0000059,0.0000076] # In case you want to set graphite boron impurity to something other than the 1 ppm CP2 default
...
MU236 = atomic_mass('U236')
MU238 = atomic_mass('U238')

def openmc_materials(*args):
...
##############################################
    ...
    mats['B4C Rod Old'] = openmc.Material(name='Pre 1960 B4C Control Rods')
    ...
    mats['B4C Rod'] = openmc.Material(name='Post 1967 B4C Control Rods')
    ...
    mats['Zirc4'] = openmc.Material(name='Zircaloy 4')
    ...
    zr3 = openmc.Material(name='Zircaloy 3')
    zr3.temperature = 300
    ...
    mats['Lead'] = openmc.Material(name='Lead B29')
    ...
    al6061 = openmc.Material(name="Aluminum 6061")
    ...
    mats['Al6063'] = openmc.Material(name='Aluminum 6063')
    ...
    mild_steel = openmc.Material(name="Mild Steel")
    ...
    mats['SS304'] = openmc.Material(name='Stainless Steel 304')
    ...
    mats['Concrete'] = openmc.Material(name='High-Density Heavy Magnetite and / or Hematite Concrete')
    ...
    mats['Graphite'] = openmc.Material(name='Chicago Pile Graphite')
...
########## Graphite with Boron Impurity #################
    ...
    # Density of graphite from Kord's document - low because of ~25% void fraction
    for boron in wBpGraphite: # wBpGraphite defined at the beginning of the file
      ...
      mats[mat_graph] = openmc.Material(name=mat_graph_name)
...
# From BATMAN Report
    ...
    for wBpFuel in wBpFuels: # wBpFuels defined at the beginning of the file

      # Calculate molar mass of Uranium
      MU = 1.0/(enr_24/MU234 + enr_25/MU235 + enr_26/MU236 + enr_28/MU238)
      ...
      mats[mat_fuel] = openmc.Material(name=mat_fuel_name)
    ...
    mats['Dysprosium'] = openmc.Material(name='Dysprosium absorber')
